 ...
 * Players navigate around countertops, appliances, and cooking stations.
 * 
 * Green tiles award crumbs (food scraps), red tiles trigger minigames, blue tiles do nothing.
 */
public class KitchenHavocBoard extends GameBoard {
    ...
    /**
     * Creates the board tiles in a kitchen layout.
     * The board represents paths around kitchen counters, appliances, and surfaces.
    ...
    public void show() {
        ...
        backgroundTex = new Texture("board/KitchenHavoc/background.png");
    ...
    private void applyTileEffect() {
        ...
        switch (currentTile.getType()) {
            case GREEN:
                ...
                statusMessage = currentPlayer.getName() + " found " + CRUMBS_REWARD + " crumbs!";
                messageTimer = 1.5f;
                state = GameState.TILE_EFFECT;
                break;
    ...
    private void draw() {
        Gdx.gl.glClearColor(0.4f, 0.35f, 0.3f, 1f);  // Kitchen brown background
    ...
    private void drawTiles(float screenWidth, float screenHeight) {
        ...
        shapeRenderer.setColor(0.2f, 0.15f, 0.1f, 0.8f);  // Dark brown for kitchen theme
        ...
        for (BoardTile tile : tiles) {
            ...
            // Draw dark outline
            shapeRenderer.setColor(0.15f, 0.1f, 0.05f, 1f);
            ...
            if (tile.getId() == shopTileIndex) {
                shapeRenderer.setColor(0.9f, 0.6f, 0.2f, 1f);  // Orange for shop (like a fruit)
            } else {
                switch (tile.getType()) {
                    case GREEN:
                        shapeRenderer.setColor(0.3f, 0.8f, 0.3f, 1f);  // Fresh green
                        break;
                    case RED:
                        shapeRenderer.setColor(0.9f, 0.3f, 0.2f, 1f);  // Hot red (like a stove)
                        break;
                    case BLUE:
                    default:
                        shapeRenderer.setColor(0.5f, 0.6f, 0.7f, 1f);  // Steel blue (like appliances)
                        break;
                }
            }
    ...
    private void drawPlayers(float screenWidth, float screenHeight) {
        ...
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        shapeRenderer.setColor(1f, 0.9f, 0.3f, 0.5f);  // Warm yellow glow
    ...
    private void drawJunctionOptions(float screenWidth, float screenHeight) {
        ...
        for (int i = 0; i < junctionOptions.size(); i++) {
            ...
            if (i == selectedJunctionIndex) {
                shapeRenderer.setColor(1f, 0.9f, 0.3f, 0.9f);  // Warm yellow
    ...
    private void drawUI(float screenWidth, float screenHeight) {
        ...
        Gdx.gl.glBlendFunc(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        shapeRenderer.setColor(0.3f, 0.2f, 0.15f, 0.8f);  // Dark brown bar
